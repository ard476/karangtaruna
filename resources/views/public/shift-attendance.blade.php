...
@section('title', 'Absen Shift')
@section('content')
<div class="w-full max-w-md rounded-2xl border bg-white p-5 shadow-sm">
    @include('components.flash')

    <div class="mb-4">
        <p class="text-xs font-semibold uppercase tracking-wide text-emerald-600">Absensi QR Shift</p>
        <h1 class="mt-1 text-xl font-bold text-slate-900">{{ $shift->nama }}</h1>
        <p class="mt-1 text-sm text-slate-600">{{ $activity->judul }}</p>
        <p class="mt-1 text-xs text-slate-500">
            {{ $shift->mulai_pada->format('d/m/Y H:i') }}
            @if($shift->selesai_pada) &mdash; {{ $shift->selesai_pada->format('H:i') }} @endif
        </p>
        @if($shift->lokasi)<p class="mt-1 text-xs text-slate-500">Lokasi: {{ $shift->lokasi }}</p>@endif
        @if($shift->hasRadius())
            <p class="mt-1 text-xs text-amber-700">Radius absen: {{ $shift->radius_meters }} meter</p>
        @endif
    </div>

    @if(! $shift->isInAttendanceWindow())
        <div class="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
            Absensi hanya aktif saat shift berlangsung.
        </div>
    @else
        <form method="POST" action="{{ route('public.shift-attendance.store', $shift->qr_token) }}" enctype="multipart/form-data" class="space-y-4">
            @csrf
            ...
            <div>
                <label class="text-sm font-medium text-slate-700">Nama anggota *</label>
                <input type="text" name="nama_lengkap" value="{{ old('nama_lengkap') }}" required autocomplete="name" placeholder="Tulis nama lengkap Anda" class="input-touch mt-1">
                <p class="mt-1 text-xs text-slate-500">Tulis nama lengkap sesuai identitas absensi.</p>
            ...
            <div>
                ...
                <p class="mb-2 text-xs text-slate-500">Foto wajib diambil dari kamera HP dan wajah harus terlihat jelas.</p>
            ...
            <div>
                ...
                <input type="text" name="keterangan" value="{{ old('keterangan') }}" maxlength="255" class="input-touch mt-1">
            </div>
