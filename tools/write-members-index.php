...
@section(\'content\')
    ...
    __OPEN__ class="card mb-4 p-4">
        <form method="GET" action="{{ route(\'admin.members.index\') }}" class="space-y-4">
            __OPEN__><label class="mb-1 block text-xs font-medium text-slate-600">Cari</label>
            <input type="text" name="q" value="{{ $filters[\'q\'] ?? \'\' }}" placeholder="Nama, NIK..." class="input-touch">__CLOSE__
            __OPEN__ class="grid gap-4 sm:grid-cols-2">
                __OPEN__><label class="mb-1 block text-xs font-medium text-slate-600">RT</label>
                ...
                __OPEN__><label class="mb-1 block text-xs font-medium text-slate-600">Status</label>
            ...
            __CLOSE__
            __OPEN__ class="grid grid-cols-2 gap-2">
    ...
    __CLOSE__
    __OPEN__ class="space-y-3 md:hidden">
        ...
        <article class="card p-4">
            ...
            __OPEN__ class="mt-3 grid grid-cols-2 gap-2">
    ...
    __CLOSE__
    __OPEN__ class="card hidden md:block"><__OPEN__ class="kt-table-wrap">
    ...
    </tbody></table>__CLOSE____CLOSE__
    @if($members->hasPages())__OPEN__ class="mt-4">{{ $members->links() }}__CLOSE__@endif
@endsection';

$content = str_replace(['__OPEN__', '__CLOSE__'], [$o, $cl], $raw);
file_put_contents(dirname(__DIR__).'/resources/views/admin/members/index.blade.php', $content);
echo "ok\n";
